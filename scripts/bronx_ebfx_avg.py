...
for facility in FACILITIES:
    ...
    for (label, year), (start, end) in WEEKS.items():
        where = f"facility = '{facility}' AND date between '{start}T00:00:00' and '{end}T23:59:59'"
        df = fetch_all_rows(where)
        if df.empty:
            print(f"  {label} {year}: NO DATA")
            continue
        df['traffic_count'] = pd.to_numeric(df['traffic_count'], errors='coerce')
        avg = df['traffic_count'].mean()
        total = df['traffic_count'].sum()
        print(f"  {label} {year}: {len(df)} rows | total = {total:,.0f} | avg/reading = {avg:.2f}")